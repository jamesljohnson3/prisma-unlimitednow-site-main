  ...
  return (
    <div key="link_logo" className="fauna-logo">
      <Link className="logo-container" to="/#home">
        <img alt="unlimitedNow logo" src="/images/home.png" />
      </Link>
    </div>
  )
}
...
const renderLoginLink = sessionContext => {
  ...
  if (user) {
    return renderLink({ handleClick: handleLogout, label: 'Create' }, sessionContext)
  } else {
    return renderLink({ href: '/accounts/login', label: 'Studio' }, sessionContext)
...
const links = [
  renderLogo,
  s => renderProtectedLink(s, { href: '/home', label: 'Connect' }),
  ...
  s => renderProtectedLink(s, { href: '/profile', label: 'Playground' }),
  renderLoginLink
]
